...
public class WorkspacePostListDao {
	...
	public ArrayList<WorkspacePostListDto> getDocument_List(int workspace_id) {
		...
		String sql = "SELECT m.name, i.kanban_icon_p, d.title, to_char(d.edit_date, '\"\"yy\"년 \"mm\"월 \"dd\"일\"') edit_date , d.document_id " +
					"FROM document d, member m, kanban_icon i " + 
					"WHERE d.writer_id = m.member_id AND d.workspace_id = ? " + 
					"AND d.kanban_icon_id = i.kanban_icon_id AND d.drafts = 0 ORDER BY d.edit_date DESC";
		try {
			...
			while(rs.next()) {
				...
				WorkspacePostListDto dto = new WorkspacePostListDto(name, kanban_icon_p, title, "", edit_date, document_id);
				list.add(dto);
			}
